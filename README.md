...
如果你**不想两层都改**——比如担心污染用户环境变量、`settings.json` 在同步盘上、或公司策略禁止改用户环境——v1.1.0 起菜单里多了细粒度选项：

| 选项 | 改什么 | 影响范围 | 典型场景 |
|---|---|---|---|
| `1) Install (both)` | settings.json **+** 用户环境变量 / shell rc | 桌面版 + 终端 | **默认推荐**，互为兜底 |
| `2) Install settings.json only` | 只改 settings.json | 桌面版（CLI 通常也会读到） | 不想动用户环境变量 |
| `3) Install user env vars only`（Windows）<br>`3) Install shell rc only`（Mac/Linux）| 只写用户环境变量 / 追加 shell rc | 只对终端启动的 Claude 生效 | 不想动 settings.json |

回滚同理（`5/6/7`）。非交互调用：`-Action install-settings` / `install-env` / `rollback-settings` / `rollback-env`（Windows），或同名子命令（Mac/Linux）。

**仍然推荐 1（两层都写）**——除非你明确知道为什么要选别的。
...
运行安装器 → 选 `5) Rollback (both)`。它会从 `settings.json` 删掉那两个键，并清掉用户环境变量 / shell rc 里对应的行（**同时恢复 auto-compact**）。每次写入前都会自动备份 `settings.json.bak.<时间戳>`。

只想撤掉其中一层？选 `6) Rollback settings.json only` 或 `7) Rollback user env vars only`（Mac/Linux 是 `Rollback shell rc only`）。
...
- **v1.1.0**（2026-06-18）—— 细粒度安装/回滚（settings-only / env-only），菜单增加白话副文案，Status 判定附"含义"行，重启提示统一加强。详见 [CHANGELOG](CHANGELOG.md)。
